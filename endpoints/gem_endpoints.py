...
def gems(lte: Optional[int] = None,
         gte: Optional[int] = None,
         type: List[Optional[GemType]] = Query(None)):

    # gems = repos.gem_repository.select_all_gem()
...
def gem(id: int):
    ...
    if not gem_found:
        return JSONResponse(content='Gem not found',
...
def create_gem(gem_pr: GemProperties, gem: Gem, user=Depends(auth_handler.get_current_user)):
    ...
    if not user.is_seller:
        return JSONResponse(content='You are not seller', status_code=HTTP_401_UNAUTHORIZED)
    gem_properties = GemProperties(size=gem_pr.size,
                                   clarity=gem_pr.clarity,
                                   color=gem_pr.color,)
    session.add(gem_properties)
    session.commit()
    gem_ = Gem(price=gem.price,
...
@gem_router.put('/gem/{id}', response_model=Gem, tags=['Gems'], )
...
def patch_gem(id: int, gem: GemPatch, user=Depends(auth_handler.get_current_user)):
    ...
    # update_data.pop('gem_properties_id', None)
